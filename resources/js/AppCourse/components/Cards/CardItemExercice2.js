...
export default function CardItemExercice2({ exercice }) {
    const dispatch = useDispatch();
    const like = useSelector(state => state.exercices.exercices.find(el => el.id === exercice.id).likes);
    const isLike = useSelector(state => state.exercices.exercices.find(el => el.id === exercice.id).isLike);
    const [onRaiting, setOnRaiting] = useState(false);
    const handleLike = async () => {
        const { id } = exercice;
        if (isLike) {
            dispatch(removeLike({ exerciceId: id }));
            dispatch(decrementLike({ id }));
        }
        else {
            dispatch(addLike({
                likeable_type: 'exercices',
                likeable_id: id
            }));
    ...
    return (<>
        <Modal
            ...
            <RaitingView model={'exercices'} courId={exercice.id} onClose={() => { setOnRaiting(false) }} />
        </Modal>
        <article className='bg-white flex flex-col h-full pointer-events-auto rounded-lg'>
            <header>
                <div className='group relative'>
                    ...
                    <div className="absolute bg-black rounded bg-opacity-0 group-hover:bg-opacity-60 w-full h-full top-0 flex items-center group-hover:opacity-100 transition justify-evenly">
                        ...
                        <Link to={`/exercices/read/${exercice.slug}-${exercice.id}`} className="hover:scale-110 text-white opacity-0 transform translate-y-3 group-hover:translate-y-0 group-hover:opacity-100 transition">
                            {switchIconsTypeexercice(exercice.contents[0].type_content, 'w-10 h-10')}
                        </Link>
                        ...
                        <button
                            ...
                            <span className='text-white font-semibold'>{exercice.rating}</span>
                        </button>
                    </div>
                </div>
            ...
            <p className='text-base text-gray-600 grow px-2'>{exercice.description}</p>
            <footer className='flex items-center mt-4 px-2 pb-2'>
                <div className="flex -space-x-1 overflow-hidden mr-1">
                    <Tooltip title={exercice.users[0].pseudo}>
                        <Link to={`/profile/user/${exercice.users[0].slug}-${exercice.users[0].user_id}`}>
                            <img
                                className="inline-block h-8 w-8 object-cover rounded-full ring-2 ring-white"
                                src={exercice.users[0].url_image} alt={exercice.users[0].pseudo} />
                        </Link>
                    </Tooltip>
                ...
                <div className="flex items-center font-medium text-xs text-ellipsis">
                    ...
                    <span className='text-gray-500'>{exercice.updated_at}</span>
                    <span className='text-gray-600'>&nbsp;</span>
                    {switchIconsTypeexercice(exercice.contents[0].type_content, 'w-4 h-4')}
                </div>
            </footer>
        </article>
    </>)
...
const switchIconsTypeexercice = (type, size) => {
