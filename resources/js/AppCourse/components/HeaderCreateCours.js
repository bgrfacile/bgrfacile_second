...
export default function HeaderCreateCours({ title, onSubmit }) {
    return <div className="flex flex-wrap justify-between items-end pb-2 border-b mb-4">
        <div>
            <h2 className="text-2xl font-bold text-gray-800">{title}</h2>
        </div>
        <div className="flex items-center">
            <button onClick={()=>onSubmit(0)} className="flex items-center px-2 py-1 bg-green-600 text-gray-100 hover:bg-green-900 border rounded-md transition ease-in-out">
            ...
            <button onClick={()=>onSubmit(1)} className="ml-2 flex items-center px-2 py-1 bg-gray-200 hover:bg-gray-800 hover:text-gray-100 border rounded-md transition ease-in-out">
